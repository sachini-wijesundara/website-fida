  ...
  useEffect(() => {
    async function fetchSolution() {
      try {
        ...
        const solution = data.find((s: any) => s.id.toString() === id);
        if (solution) {
          setFormData({
            title: solution.title,
            description: solution.description || "",
            iconName: solution.icon_name || "Zap",
            orderIndex: solution.order_index.toString(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || "Failed to update solution.");
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="h-96 flex flex-col items-center justify-center gap-4 text-[var(--text-muted)]">
        ...
        <p className="text-sm font-medium">Loading solution data...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-20">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-bold tracking-tight text-white">Edit Solution</h2>
            <p className="text-[var(--text-secondary)] mt-1">Update details for {formData.title}.</p>
          </div>
        </div>
        <button 
        ...
        >
          ...
          {saving ? "Updating..." : "Update Solution"}
        </button>
      </header>
